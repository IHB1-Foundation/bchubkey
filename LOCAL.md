# 로컬 실행 가이드 (Postgres + Docker)

이 프로젝트는 **Prisma + Postgres**를 사용합니다. 로컬에서는 `docker-compose.yml`로 Postgres를 띄우고, Prisma 마이그레이션을 적용한 뒤 봇을 실행하면 됩니다.
...
```bash
docker compose exec postgres psql -U bchubkey -d bchubkey
```
...
### Postgres가 아직 준비가 안 됨

`prisma migrate`가 연결 실패하면 Postgres가 아직 `healthy`가 아닐 수 있습니다.
...
다른 Postgres가 이미 5432를 쓰고 있으면 `docker-compose.yml`의 포트 매핑을 바꾼 뒤, `DATABASE_URL`도 같은 포트로 맞추세요.

예: `5433:5432`로 바꾸면

```env
DATABASE_URL=postgresql://bchubkey:bchubkey@localhost:5433/bchubkey
```
